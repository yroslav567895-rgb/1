...
function showPage(name) {
    Object.keys(pages).forEach(k => {
        pages[k].classList.remove('active', 'visible');
    });
    if (pages[name]) {
        pages[name].classList.add('active');
        requestAnimationFrame(() => pages[name].classList.add('visible'));
    }
    currentPage = name;
    updateNav();
}
...
function updateNav() {
    ...
    if (state.role === 'admin' || state.role === 'superadmin') {
        html += `<button class="nav-btn ${currentPage === 'admin' ? 'active' : ''}" onclick="goAdmin()">Admin</button>`;
    }
...
$('loginForm').addEventListener('submit', async (e) => {
    ...
    if (ok) {
        ...
        showStatus('loginStatus', 'Welcome, ' + data.username + '!', 'success');
...
$('loginRemember').addEventListener('change', function() {
    // Visual feedback is automatic via CSS
});
...
async function loadKeys() {
    ...
    state.keys = data;
    if (!data.length) {
        container.innerHTML = '<div class="no-keys">No keys activated yet. Enter your license key above.</div>';
        $('subscriptionCard').style.display = 'none';
        return;
    }

    $('subscriptionCard').style.display = 'block';
    // Show the first active key's info
...
function renderSubInfo(key) {
    ...
    $('subInfo').innerHTML = `
        <div class="sub-item">
            <div class="sub-item-label">Status</div>
            <div class="sub-item-value ${isActive ? 'active' : 'expired'}">${isActive ? 'Active' : 'Expired'}</div>
        </div>
        <div class="sub-item">
            <div class="sub-item-label">Created</div>
            <div class="sub-item-value">${formatDate(key.createdAt)}</div>
        </div>
        <div class="sub-item">
            <div class="sub-item-label">Expires</div>
            <div class="sub-item-value ${isActive ? 'active' : 'expired'}">${formatDate(key.expiresAt)}</div>
        </div>
        <div class="sub-item">
            <div class="sub-item-label">Key</div>
            <div class="sub-item-value" style="font-size:13px;font-family:monospace;">${key.key}</div>
        </div>
    `;
}
...
async function loadModVersion() {
    ...
    if (data && data.version) {
        $('modVersionInfo').innerHTML = `<div class="mod-version">Version: ${data.version}</div>`;
    } else {
        $('modVersionInfo').innerHTML = `<div class="mod-version" style="color:var(--orange);">No mod uploaded yet</div>`;
    }
}
...
$('claimKeyBtn').addEventListener('click', async () => {
    ...
    if (ok) {
        showStatus('dashStatus', 'Key activated! Subscription until: ' + formatDate(data.expiresAt), 'success');
        $('keyInput').value = '';
        loadKeys();
    } else {
        showStatus('dashStatus', data.error || 'Activation failed', 'error');
    }
});
...
$('downloadBtn').addEventListener('click', () => {
    window.location.href = '/api/download-mod';
});
...
async function loadOnlineUsers() {
    ...
    if (ok) {
        $('onlineUsers').innerHTML = `
            <div style="font-size:36px;font-weight:800;color:var(--accent);">${data.count}</div>
            <div style="font-size:13px;color:var(--text-dim);margin-top:4px;">users online (5 min)</div>
            <div style="font-size:12px;color:var(--text-dim);margin-top:8px;">${data.users.join(', ') || '—'}</div>
        `;
    } else {
        $('onlineUsers').innerHTML = '<div style="color:var(--red);">Access denied</div>';
    }
}
...
async function loadUsersTable() {
    ...
    let html = '<table class="admin-table"><thead><tr><th>Username</th><th>Role</th><th>Created</th><th>Status</th><th>Actions</th></tr></thead><tbody>';
    data.forEach(u => {
        const roleBadge = u.role === 'superadmin' ? 'badge-superadmin' : u.role === 'admin' ? 'badge-admin' : 'badge-user';
        html += `<tr>
            ...
            <td>
                <div class="admin-actions">
                    ${u.role !== 'superadmin' ? `
                        <button class="btn btn-sm ${u.banned ? 'btn-green' : 'btn-red'}" onclick="toggleBan('${u.username}', ${!u.banned})">
                            ${u.banned ? 'Unban' : 'Ban'}
                        </button>
...
async function toggleBan(username, ban) {
    ...
    if (ok) { loadUsersTable(); showStatus('adminStatus', ban ? 'User banned' : 'User unbanned', 'success'); }
...
async function loadKeysTable() {
    ...
    if (!data.length) {
        html += '<tr><td colspan="6" style="text-align:center;color:var(--text-dim);">No keys</td></tr>';
    }
    data.forEach(k => {
        ...
        html += `<tr>
            ...
            <td>${k.claimedBy || '<span style="color:var(--text-dim);">—</span>'}</td>
            <td style="color:var(--text-dim);">${formatDate(k.createdAt)}</td>
            <td style="color:var(--text-dim);">${formatDate(k.expiresAt)}</td>
            <td><span class="badge ${statusClass}">${status}</span></td>
            <td><button class="btn btn-sm ${k.active ? 'btn-red' : 'btn-green'}" onclick="toggleKey('${k.key}', ${!k.active})">${k.active ? 'Deactivate' : 'Activate'}</button></td>
        </tr>`;
    });
    html += '</tbody></table>';
...
async function toggleKey(key, active) {
    ...
    if (ok) { loadKeysTable(); showStatus('adminStatus', 'Key ' + (active ? 'activated' : 'deactivated'), 'success'); }
}
...
async function loadActivity() {
    ...
    if (!data.length) {
        html += '<tr><td colspan="3" style="text-align:center;color:var(--text-dim);">No activity</td></tr>';
    }
    data.slice().reverse().forEach(a => {
        html += `<tr>
            <td style="color:var(--text-dim);font-size:12px;">${formatDate(a.timestamp)}</td>
            <td>${a.username}</td>
            <td style="color:var(--text-dim);">${a.details || a.action}</td>
        </tr>`;
    });
    html += '</tbody></table>';
...
$('createKeyBtn').addEventListener('click', async () => {
    ...
    if (ok) {
        $('newKeyResult').innerHTML = `Key: <strong>${data.key}</strong><br>Expires: ${formatDate(data.expiresAt)}`;
        loadKeysTable();
        showStatus('adminStatus', 'Key created!', 'success');
    } else {
