...
async function loadKeysTable() {
    ...
    data.forEach(k => {
        ...
        html += `<tr><td style="font-family:monospace;font-size:12px;">${k.key}</td><td>${k.claimedBy || '<span style="color:var(--text-dim);">—</span>'}</td><td style="color:var(--text-dim);font-size:12px;">${formatDate(k.createdAt)}</td><td style="color:var(--text-dim);font-size:12px;">${formatDate(k.expiresAt)}</td><td><span class="badge ${statusClass}">${status}</span></td><td><button class="btn btn-sm ${k.active ? 'btn-red' : 'btn-green'}" onclick="toggleKey('${k.key}', ${!k.active})">${k.active ? 'Deactivate' : 'Activate'}</button></td></tr>`;
    });
    html += '</tbody></table>';
